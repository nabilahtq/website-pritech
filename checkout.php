...
<body>
    ...
    if(isset($_GET['course_id'])){
      ...
      if(!empty($id)){
        $sql = mysqli_query($koneksi,"SELECT * FROM course INNER JOIN tutors ON course.tutor_id = tutors.tutor_id 
            INNER JOIN kategori_ahli ON course.ahli_id = kategori_ahli.ahli_id WHERE course_id = '$id'") or die(mysqli_error());
    ...
    include "koneksi.php";
        if(isset($_POST['submit'])){
            ...
            INNER JOIN tutors ON daftar_murid.tutor_id = tutors.tutor_id";
            $cek= mysqli_query($koneksi, $query) or die (mysqli_error($koneksi));

            if(mysqli_num_rows($cek) > 0){
                $sql = mysqli_query($koneksi, "INSERT INTO daftar_murid(id_murid, name, contact, email, judul, course_id,tutor_id, nama_tutor,tanggal, tanggal_end, harga, metodepay) VALUES('$id_murid', '$name', '$contact', '$email', '$judul', '$course_id','$tutor_id', '$nama_tutor', '$tanggal', '$tanggal_end', '$harga', '$metodepay')") or die(mysqli_error($koneksi));

                if($sql){
                    echo '<script>alert("Berhasil melakukan Transaksi."); document.location="konfirmasi.php?konfirmasi=pembayaran";</script>';
                }else{
    ...
    <div class="row justify-content-center">
      <div class="col-lg-6 px-4 pb-4" id="order">
        ...
        <div class="jumbotron p-3 mb-2 text-center">
          <h5 ><b>Training : </b><?php echo $data['judul']; ?></h6>
          <h6 ><b>Tanggal : </b><?php echo date('d F Y', strtotime($data["tanggal"]));?> - <?php echo date('d F Y', strtotime($data["tanggal_end"]));?></h6>
          <h6><b>Total Harga : </b>Rp.<?= number_format($data['harga_diskon'],2) ?></h5>
        </div>
        <form action="" method="post" role="form">
          <input type="hidden" name="judul" value="<?php echo $data['judul']; ?>">
          <input type="hidden" name="tutor_id" value="<?php echo $data['tutor_id']; ?>">
          <input type="hidden" name="course_id" value="<?php echo $data['course_id']; ?>">
          <input type="hidden" name="nama_tutor" value="<?php echo $data['nama']; ?>">
          <input type="hidden" name="tanggal" value="<?php echo $data['tanggal']; ?>">
          <input type="hidden" name="tanggal_end" value="<?php echo $data['tanggal_end']; ?>">
          <input type="hidden" name="harga" value="<?php echo $data['harga_diskon']; ?>">
          <div class="form-group">
            <input type="hidden" name="id_murid" class="form-control" placeholder="Enter Name" required>
          </div>
          <div class="form-group">
          ...
          <div class="form-group">
            <input type="phone" name="contact" class="form-control" placeholder="Enter Phone" required>
          </div>
          <h6 class="text-center lead">Select Payment Mode</h6>
          <div class="form-group">
            <select name="metodepay" class="form-control">
          ...
          <div class="form-group">
            <center>
            <button type="submit" name="submit" id="submit" class="btn btn-warning btn-block" title="bayar"> <i class="glyphicon glyphicon-floppy-disk"></i> Place Order</button></center>
          </div>
        </form>
      </div>
    </div>
